...
| `elevation` | `f64?` | Ground elevation (m MSL) at the primary location (falling back to `lat`/`lng` for records that predate primary tracking), resolved via the terrain service during rollup; cleared when the station moves. Required for horizon output. The Terrarium DEM includes ocean bathymetry, so all elevation lookups clamp to a per-location floor: sea level everywhere except a small built-in table of genuine below-sea-level land regions (Dead Sea, Caspian shore, Death Valley, Dutch polders, ...), each floored at its known lowest surface |
| `beaconAltitude` | `f64?` | Antenna altitude (m MSL) from the station's location beacons (`/A=`). Only packets at the **primary location** update it — a bouncing partner's beacon (two receivers sharing a callsign) is ignored so it can't churn the ground-horizon file. A location rotation replaces it with the new site's value, clearing it when the moving beacon carries no altitude. The horizon viewpoint uses `beaconAltitude − elevation` when that difference is 0–300 m, otherwise the `GROUND_STATION_AGL_M` default (3 m; the ground-horizon file then records `stationAgl` as `NaN`) |
| `groundHorizonBeacon` | `f64?` | Beacon altitude (m MSL, raw `/A=` input) the ground-horizon file was generated with; `None` = no beacon known then. A ≥2 m difference from the live `beaconAltitude` (or a beacon appearing/disappearing) re-queues the terrain file, so a corrected antenna altitude takes effect without a move. Cleared on move alongside `groundHorizonPos` |
| `groundHorizonPos` | `[f64, f64]?` | Position the ground-horizon terrain file was generated for (the primary location at generation time); cleared when the station moves so the file is regenerated |
