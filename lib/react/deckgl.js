...
//
function makeLayers( station, setStation, map2d ) {
	...
	let layers = [
		new H3HexagonLayer({
			id: station||'global',
			data: '/data/'+(station||'global')+'/avgSig.arrow',
			loaders: [ ArrowLoader ],
//			loadOptions: {
//				arrow: {
//				}
			...
			dataTransform: (d) => {
				...
				return _map( stringed, (v,i) => { return { h: v, v: d.value[i] }})},
			pickable: false,
			wireframe: false,
			filled: true,
			extruded: false,
			elevationScale: 20,
			getHexagon: (d) => d.h,
			getFillColor: d => [255, (1 - d.v/ 255) * 255, 0, 125],
			getElevation: d => 0
		}),
...
export default function CoverageMap(props) {
	...
	const layers = useMemo( _ => makeLayers(props.station, props.setStation, map2d),
							[ props.station, map2d ]);
