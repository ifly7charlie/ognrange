...
const visualisationFunctions = {
	...
	'stations': (f) => colourise(f.t*10),
	'primaryStation': (f) => [ parseInt(f.s.slice(0,1),36)*7,  parseInt(f.s.slice(1,2),36)*7,  parseInt(f.s.slice(2,3),36)*7, 128 ],
};
...
export function CoverageMap(props) {
	...
	function useToolTipSelection({object,picked}) {
		...
		if( props.tooltips ) {
			const html = ReactDOMServer.renderToStaticMarkup( <CoverageDetails details={object}
...
export function CoverageDetails( {details,setHighlightStations,highlightStations} ) {
