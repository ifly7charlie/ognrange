...
//
function makeLayers( station, file, setStation, highlightStations, visualisation, map2d, onClick, lockedH3, setProgress ) {
	...
	const hexLayer = new H3HexagonLayer({
		...
		loadOptions: {
			fetch: (...o)=>{return fetch(...o).then(progressFetch(setProgress));} 
		},
		loaders: ArrowLoader,
			dataTransform: (d) => {
				...
				const r = _map( stringed, (v,i) => { maxCount = Math.max(maxCount,d.count[i]);
												  return {
													  ...
													  p: d.avgGap[i],
													  q: d.expectedGap?.[i],
													  s: d.stations ? d.stations[i] : 0,
													  t: d.stations ? (d.stations[i]?.split(',')?.length||0) : 0
												  }}
							  );
...
export function CoverageMap(props) {
	...
	const { hexLayer, layers } =
		  useMemo( _ => makeLayers(props.station, props.file, props.setStation, props.highlightStations,
								   props.visualisation, map2d, onClick, lockedH3, setLoaded ),
	...
	const loadingLayer = isLoaded ?(<div className='progress-bar'>
...
export function CoverageDetails( {details,station,setHighlightStations,highlightStations} ) {
	...
	if( details.h ) {
		return <div>
				   ...
				   <hr/>Avg Gap: {details.p>>2}s {(((details.q??true) !== true)&& stationCount > 1? <>(expected: {details.q>>2}s)<br/></> : <br/>)}
