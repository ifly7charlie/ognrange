                ...
                {details.locked && byDay ? ( //
                    <VisibilitySensor onChange={setExtraVisible}>
                        <>
                            ...
                            {extraVisible ? ( //
                                <OtherStationsDetails h={details.h} file={file} station={station} />
                            ) : null}
                        </>
                    </VisibilitySensor>
                ) : null}
...
function OtherStationsDetails(props) {
    ...
    const {data: byDay, error} = useSWR(
        props.station + props?.h == doFetch && props?.h //
            ? `/api/station/${props.station || 'global'}/h3others/${prefixWithZeros(8, props.h[1].toString(16))}${prefixWithZeros(8, props.h[0].toString(16))}?file=${props.file}`
            : null,
        fetcher
    );
    ...
    return (
        <>
            Top 5 other stations total packet count:
            <br />
            {byDay?.data?.length > 0 ? (
                <>
