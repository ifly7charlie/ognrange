...
export function CoverageDetails({details, station, setHighlightStations, highlightStations, file}) {
    ...
    const {data: byDay, error} = useSWR(
        station + details?.h == doFetch && details?.h //
            ? `/api/station/${station || 'global'}/h3details/${details.h[1].toString(16)}${details.h[0].toString(16)}?file=${file}`
            : null,
        fetcher
    );
    ...
    return (
        <>
            ...
            {props.byDay && props.byDay.length > 0 ? (
                <>
                    <br />
                    <LineChart width={480} height={180} data={props.byDay} margin={{top: 5, right: 30, left: 20, bottom: 5}}>
            ...
            {props.byDay && props.byDay.length > 0 ? (
                <>
                    ...
                    <LineChart width={480} height={180} data={props.byDay} margin={{top: 5, right: 30, left: 20, bottom: 5}}>
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="date" />
                        <YAxis unit="dB" yAxisId={0} orientation="left" />
                        <YAxis unit=" m" yAxisId={1} orientation="right" />
                        <Tooltip />
                        <Legend />
