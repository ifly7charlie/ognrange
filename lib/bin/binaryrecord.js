...
export class BinaryRecord {
	...
	//
	// Append data to arrow output buffer {
	output(arrow) {
		this._sh.output(arrow);
	}
	...
	appendToArrow(h3,arrow) {
		...
		if( this._sh.nestedVersion ) {
			...
			while( i != 0 && sid != 0 ) {
				...
				o = ((o ? o+',' : '') + (((sid*10)+Math.round((10*scount)/count)).toString(36)));
...
////////////////////////////////////
	...
	_update( o8, o16, o32, sh, altitude, agl, crc, signal, count = 1 ) {
		...
		const expandedSignal = signal * 4;
		if( this._u8[o8 + sh.u8oMaxSig] < expandedSignal ) {
			this._u8[o8 + sh.u8oMaxSig] = expandedSignal;
		}

		this._u32[o32 + sh.u32oSumSig] += signal;
