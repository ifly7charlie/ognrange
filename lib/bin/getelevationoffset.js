...
const options = { max: parseInt(process.env.MAX_ELEVATION_TILES)||7200,
				  dispose: function (n,key) { console.log( `flushed elevation ${key} from cache [${cache.getTtl()}s ttl remaining]` ); },
				  updateAgeOnGet: true, allowStale: true,
				  ttl: (process.env.ELEVATION_TILE_EXPIRY_HOURS||36) * 3600 * 1000 }
, cache = new LRU(options)

cache.getTtl = (k) => {
	return (typeof performance === 'object' && performance &&
	 typeof performance.now === 'function' ? performance : Date).now() - cache.starts[cache.keyMap.get(k)]
}
    ...
    if( ! pixels ) {
        ...
        fetch( url, { "headers": { "Referer":referrer }} )
            .then( (res) => {
				if( res.status != 200 ) {
					if( res.status != 404 && res.status != 422 ) {
						throw `MapBox API returns ${res.status}: ${res.statusText}, ensure the value for NEXT_PUBLIC_SITEURL set in .env.local is in the allowed ACL on mapbox, (lat:${lat},lng:${lng})`;
					}
					else {
						throw undefined;
					}
            ...
            .catch(err => {
                ...
                if( err ) { console.error("unable to read elevation: ",err,url); }
