...
export class CoverageRecord {
	...
	updateSumDelay( number ) {
		this._u32[this._sh.u32oSumDelay] += number;
	}
...
//////////////////////////////
	...
	static initArrow(type) {
		let arrow = {
			...
			avgDelay: makeBuilder({type: new Uint8()}),
		}
		if( type == bufferTypes.global ) {
			arrow.stations = makeBuilder({type: new Utf8()});
			arrow.expectedDelay = makeBuilder({type: new Utf8()});
		}
		return arrow;
	}
	...
	appendToArrow(h3,arrow) {
		...
		arrow.avgDelay?.append((this._u32[this._sh.u32oSumDelay]/count));
		...
		if( this._sh.nestedVersion ) {
			...
			arrow.expectedDelay.append((((this._u32[this._sh.u32oSumDelay]/count)*10)/ns));
		}
	}

	//
	// The last action we need to be able to do is fold our data into another
	// CoverageRecord to allow aggregation. In this situation we are read only
	// the destination will be updated with our value and any stations no longer
	// listed will be removed from it
	//
	// NOTE: this can return a new CoverageRecord which replaces both src and dest!
	//
	...
	rollup( /*CoverageRecord*/ srcRecord, validStationSet ) {
		...
		// of stationid:index
		const destStations = CoverageRecord._makeStationMap( this );
		const srcStations = CoverageRecord._makeStationMap( srcRecord );
		...
		for ( const k in destStations ) {
			if( ! validStationSet.has(currentStations[k].id) ) {// no longer valid
				delete destStations[k]
			}
			else if( srcStations[k] ) { // id in both both so we pull them together
				destStations[k].c += srcStations[k].c; destStations[k].di = destStations[k].i; destStations[k].si = srcStations[k].i; destStations[k].i = undefined; 
		...
		for ( const pick of pickList ) {
			...
			if( pick.si || pick.i ) {
				CoverageRecord._updateNested( newBr, i, srcRecord, pick.si || pick.i );
			}
	...
	removeInvalidStations(validStationSet) {
		...
		const currentStations = CoverageRecord._makeStationMap( this );
		...
		let someToRemove = false;
		for ( const k in currentStations ) {
			if( ! validStationSet.has(currentStations[k].id) ) { delete currentStations[k]; someToRemove = true }
		}
		...
		// Sort the pick list by size so we are optimal on writing but also, if there is nothing
		// to pick then we should delete the record so return null indicating nothing found
		const pickList = _sortby([...Object.values(currentStations)], (o)=>-o.c);
		if( ! pickList.length ) {
			return null;
		}
	...
	_removeStationFromList( stationid ) {
		...
		while( i != 0 ) {
			...
			if( sid != stationid ) {
				...
				CoverageRecord._updateNested( this, 0, this, i );
			}
...
////////////////////////////////////
	...
	static _updateNested (dest,di,src,si) {
		...
		dest._u32[do32+dsh.u32oSumDelay] += src._u32[so32+ssh.u32oSumDelay];
	}
