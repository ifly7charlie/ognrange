...
//			console.log( ' * not nested' );
		...
		// of stationid:index
		const destStations = CoverageRecord._makeStationMap( this, 'di' );
		...
		for ( const k in destStations ) {
			if( ! validStationSet.has(destStations[k].id) ) {// no longer valid
				...
				destStations[k].c += srcStations[k].c; destStations[k].si = srcStations[k].si; 
...
//				console.log( pick, 'di', JSON.stringify( newBr.toObject()))
			}
			if( pick.si ) {
				CoverageRecord._updateNested( newBr, i, srcRecord, pick.si );
//				console.log( pick, 'si', JSON.stringify( newBr.toObject()))
			}
			// And then roll those up to the top level
			CoverageRecord._updateNested( newBr, 0, newBr, i );
//			console.log( pick, 'sum', JSON.stringify( newBr.toObject()))
...
//			console.log( ' * not nested' );
		...
		const currentStations = CoverageRecord._makeStationMap( this, 'i' );
