...
import { ROLLUP_PERIOD_MINUTES, STATION_EXPIRY_TIME_SECS,
	...
	if( oldAccumulator ) {
		...
		flushDirtyH3s({globalDb, stationDbCache, stations, allUnwritten:true }).then( (s) => {
			...
			rollupAll( { current: oldAccumulator, processAccumulators: oldAccumulators,
						 globalDb, statusDb, stationDbCache, stations, newAccumulator: true } );
		})
...
export async function rollupAll( { current = currentAccumulator,
	...
	promises.push( mapAllCapped( Object.keys(stations), async function (station) {
		...
		if( ! db ) {
			stationDbCache.set(stations[station].id, db = LevelUP(LevelDOWN(DB_PATH+'/stations/'+station)));
		...
		const r = await rollupDatabase( { ...commonArgs, db, stationName:station, stationMeta: stations[station] } );
		rollupStats.last.sumElapsed += r.elapsed;
		rollupStats.last.operations += r.operations;
		rollupStats.last.databases ++;
	...
	// And the global json
	promises.push( produceStationFile( stations ) );
...
export async function rollupDatabase( { db, stationName, stationMeta = {}, validStations, current=currentAccumulator, processAccumulators=accumulators } ) {
	...
	const log = ()=>false; //stationName == 'airs12345' ? console.log : ()=>false;
			...
			for( const r of rollupData ) {
				...
				if( ! prefixedh3r ) {
					...
					if( r.lastCopiedH3p != h3p.lockKey ) {
						...
						r.stats.h3missing++;
						log('       h3missing (end)', r.type, h3p.dbKey()); 
					}
				...
				if( ordering < 0 ) {
					if( r.lastCopiedH3p != h3p.lockKey ) {
						...
						r.lastCopiedH3p = h3p.lockKey;
						r.stats.h3missing++;
						log('       h3missing', r.type, (h3p.dbKey())); 
					}
					continue;
				}
	...
	for( const r of rollupData ) {
		...
		writeFile( OUTPUT_PATH + accumulatorName + '.json',
				   JSON.stringify(r.meta,null,2), (err) => err ? console.log("rollup json metadata write failed",err) : null)
		
		// link it all up for latest
		symlink( OUTPUT_PATH+accumulatorName+'.arrow', OUTPUT_PATH+`${name}/${name}.${r.type}.arrow` );
...
//		}
	...
	writeFile( OUTPUT_PATH+name+'.json', JSON.stringify(stationMeta,null,2), (err) => err ? console.log("stationmeta write error",err) : null);
...
// back to the disk
async function produceStationFile( stations ) {

	// Form a list of hashes
	let statusOutput = _filter( stations, (v) => {return v.valid});

	// Write this to the stations.json file
	writeFile( OUTPUT_PATH + 'stations.json', JSON.stringify(statusOutput,null,2), (err) => err ? console.log("station write error",err) : null);
}
