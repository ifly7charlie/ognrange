...
export async function updateAndProcessAccumulators({globalDb, statusDb, stationDbCache, stations}) {
    ...
    if (oldAccumulator) {
        ...
        flushDirtyH3s({globalDb, stationDbCache, stations, allUnwritten: true}).then((s) => {
            ...
            console.log(s);
            rollupAll({current: oldAccumulator, processAccumulators: oldAccumulators, globalDb, statusDb, stationDbCache, stations, newAccumulatorFiles: !_isequal(accumulators, oldAccumulators)});
        });
    }
    ...
    if (!_isequal(accumulators, oldAccumulators) || !_isequal(currentAccumulator, oldAccumulator)) {
        ...
        writeFileSync(OUTPUT_PATH + 'current.txt', currentFileList);
    }
}
...
export async function rollupStartup({globalDb, statusDb, stationDbCache, stations}) {
    ...
    // this makes it easier to check what needs to be done?
    {
        ...
        for (const type in expectedAccumulators) {
            ...
            globalDb
                ...
                .then((value) => {
                    const meta = JSON.parse(String(value));
                    globalDb.put(dbkey, JSON.stringify({...meta, currentAccumulator: currentAccumulatorHeader.bucket}));
                })
                .catch((e) => {
                    globalDb.put(dbkey, JSON.stringify({start: Math.floor(now / 1000), startUtc: now.toISOString(), currentAccumulator: currentAccumulatorHeader.bucket}));
                });
        }
    }
    ...
    if (hangingCurrents) {
        ...
        for (const key in hangingCurrents) {
            ...
            if (Object.keys(rollupAccumulators).length) {
                ...
                await rollupAll({current: [hangingHeader.type, hangingHeader.bucket], processAccumulators: rollupAccumulators, globalDb, statusDb, stationDbCache, stations, newAccumulatorFiles: true});
            } else {
                console.log(`purging hanging current accumulator ${hangingHeader.accumulator} and associated sub accumulators`);
            }

            // now we clear it
            await globalDb.clear(CoverageHeader.getDbSearchRangeForAccumulator(hangingHeader.type, hangingHeader.bucket, true), (e) => {
    ...
    for (const key in accumulatorsToPurge) {
        ...
        await globalDb.clear(CoverageHeader.getDbSearchRangeForAccumulator(t, b, true), (e) => {
...
export async function rollupAll({current = currentAccumulator, processAccumulators = accumulators, globalDb, statusDb, stationDbCache, stations, newAccumulatorFiles = false}) {
    ...
    rollupStats = {...rollupStats, lastStart: Date.now(), last: {sumElapsed: 0, operations: 0, databases: 0, skippedStations: 0, accumulators: processAccumulators, current: CoverageHeader.getAccumulatorMeta(...current).accumulator}};
    ...
    await purgeOldAccumulators(stations, stationDbCache);
...
export async function purgeOldAccumulators(stations, stationDbCache) {
    return false;
    // each of the stations, capped at 20% of db cache (or 30 tasks) to reduce risk of purging the whole cache
    // mapAllCapped will not return till all have completed, but this doesn't block the processing
    // of the global db or other actions.
    ...
    await mapAllCapped(
        ...
        async function (stationMeta) {
            let db = stationDbCache.get(stationMeta.id);
            if (!db) {
                stationDbCache.set(stationMeta.id, (db = LevelUP(LevelDOWN(DB_PATH + '/stations/' + stationMeta.station))));
                db.ognInitialTS = Date.now();
                db.ognStationName = stationMeta.station;
            }
            ...
            while ((y = await n.next())) {
                ...
                dbAccumulators.push(hr.accumulator);
                // Seek to the next possible bucket
                n.seek(CoverageHeader.getAccumulatorEnd(hr.type, hr.bucket));
            }

            for (const hr in dbAccumulators) {
                if (accumulators[hr.typeName].bucket != hr.bucket) {
                    await db.clear(CoverageHeader.getDbSearchRangeForAccumulator(hr.type, hr.bucket, true), (e) => {
                        console.log(`${stationMeta.station}: ${hr.accumulator} purge completed ${e || 'successfully'}`);
                    });
                } else {
                    console.log(`${stationMeta.station}: ${hr.accumulator} retained`);
                }
            }
        },
...
export function updateGlobalAccumulatorMetadata({globalDb, currentAccumulator = getAccumulator(), allAccumulators = accumulators}) {
    ...
    for (const type in allAccumulators) {
        ...
        globalDb
            ...
            .then((value) => {
                const meta = JSON.parse(String(value));
                globalDb.put(dbkey, JSON.stringify({...meta, accumulators: allAccumulators, currentAccumulator: currentAccumulator[1]}));
            })
            .catch((e) => {
                globalDb.put(dbkey, JSON.stringify({start: Math.floor(now / 1000), startUtc: now.toISOString(), accumulators: allAccumulators, currentAccumulator: currentAccumulator[1]}));
            });
    }
}
