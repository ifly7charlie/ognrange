...
export async function rollupDatabase( { db, stationName, stationMeta = {}, validStations, current=currentAccumulator, processAccumulators=accumulators } ) {
	...
	try {
		writeFileSync( OUTPUT_PATH+name+'.json', JSON.stringify(stationMeta,null,2));
	} catch (err) {
...
function produceStationFile( stations ) {
	...
	let nameList = _map( statusOutput, (v) =>v.station);

	// Write this to the stations.json file
	try {
		writeFileSync( OUTPUT_PATH + 'stations.json', JSON.stringify(statusOutput,null,2) );
		writeFileSync( OUTPUT_PATH + 'station-list.json', JSON.stringify(nameList,null,2) );
	}
	catch(err) {
