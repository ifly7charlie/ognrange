...
export class CoverageHeader {
	...
	get h3bigint() {
		return BigInt(CoverageHeader._swap32(this._u32[header.u32oH3lo]))<<32n|BigInt(CoverageHeader._swap32(this._u32[header.u32oH3hi]));
	}
	...
	static getDbSearchRangeForAccumulator(t,b) {
		...
		return { gte: Buffer.from(l.dbKey), lt: Buffer.from(h.dbKey) };
	}

	// Compares in db order so by bytes, yes please make this better I'm tired
	static compare(a,b) {
		if (a._u32[0] < b._u32[0]) { return -1 };
		if( a._u32[0] > b._u32[0]) { return 1 };
		if (a._u32[1] < b._u32[1]) { return -1 };
		if( a._u32[1] > b._u32[1]) { return 1 };
		if (a._u32[2] < b._u32[2]) { return -1 };
		if( a._u32[2] > b._u32[2]) { return 1 };
		return 0;
	}
	...
	static newAccumulator(ch,t,b) {
		const n = new CoverageHeader(0,ch);
		const _u8 = new Uint8Array( n._u32.buffer, 0, header.length );
...
//////////////////////////////
	...
	_initFromLockKey(k) {
		...
		this._u32.set( CoverageHeader._h3SwapOrder(...h3IndexToSplitLong(parts[2])), header.u32oH3lo );
		this._lockKey = k;
	}
