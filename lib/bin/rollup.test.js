...
import { getAccumulator, getAccumulatorForType,
		 ...
		 rollupAll, rollupDatabase, updateAndProcessAccumulators, rollupStartup, rollupStats } from '../lib/bin/rollup.js';
...
function set( { h3, altitude, agl, crc, signal, gap, stationid = 0, accumulator = getAccumulator(), now = Date.now() } ) {
	...
	console.log( altitude, agl, crc, signal, gap, stationid );
...
async function get( { h3, type, accumulator = null } ) {
	const acc = accumulator || getAccumulatorForType(type);
...
let db = LevelUP(LevelDOWN(DB_PATH+'global'))
...
function doTestMergeUniqueH3s( { name, input, merge, output, finish, vs = validStations } ) {
...
function doTestStartup( { name, inputAccumulators, input, currentAccumulators, output, finish } ) {
	...
	test( name+' (set metadata)', async (t) => {
		await updateGlobalAccumulatorMetadata( {globalDb: db, currentAccumulator: inputAccumulators['current'],
												allAccumulators: _filter(inputAccumulators,(o)=>(o.type!='current')) });
		t.pass();
	})
	...
	test( name+' (startup/rollup)', async(t) => {
		await rollupStartup( { globalDb: db, statusDb: null, stationDbCache: null, stations: {} });
		t.pass();
	});
