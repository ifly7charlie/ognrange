...
import { getAccumulator, getAccumulatorForType,
		 updateGlobalAccumulatorMetadata,
		 rollupAll, rollupDatabase, updateAndProcessAccumulators, rollupStartup, rollupStats } from './rollup.js';
...
async function get( { h3, type, accumulator = null, inputAccumulators = null } ) {
	const acc = accumulator ? accumulator :
				(inputAccumulators ? [ type, inputAccumulators[type].bucket ] : getAccumulatorForType(type));
...
test( 'rollup one item empty db', async (t) => {
	let meta = {};
	return rollupDatabase( {db, stationName: 'global', stationMeta: meta, validStations })
		.then((data) => {
			if( meta.accumulators['day'].h3source == 1 &&
				meta.accumulators['day'].h3missing == 1 && 
				meta.accumulators['month'].h3missing == 1 &&
				meta.accumulators['year'].h3missing == 1 ) {
				t.pass();
			}
			else {
				t.fail();
...
//	await set({ h3: '87088619bffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, stationid: 1 });
	return get( { h3: '87088619bffffff', type: 'month' } )
		.then( (d) => {
			t.is( d.MaxSig, 10 );
			t.is( d.SumGap, 1 );
...
function doTestMergeUniqueH3s( { name, input, merge, output, finish, vs = validStations, inputAccumulators = undefined, emptyDb = false, stationName = 'global' } ) {
	...
	test( name+' (rollup)', async(t) => {
		let meta = {};
		return rollupDatabase( {db, stationName, stationMeta: meta, validStations:vs, needValidPurge: vs.size > 0,
								current: inputAccumulators ? inputAccumulators['current'] : undefined,
								processAccumulators: inputAccumulators ? _reduce(inputAccumulators,(a,v,k)=>{ if(k!='current') { a[k] = v } return a}, {} ) : undefined })
			.then((data) => {
				merge.forEach( (r) => {
...
function doTestStartup( { name, inputAccumulators, input, currentAccumulators, output, finish } ) {
	...
	test( name+' (set metadata)', async (t) => {
		await updateGlobalAccumulatorMetadata( {globalDb: db, currentAccumulator: inputAccumulators['current'],
												current: inputAccumulators ? inputAccumulators['current'] : undefined,
												allAccumulators: _reduce(inputAccumulators,(a,v,k)=>{ if(k!='current') { a[k] = v } return a}, {} )
												});
		t.pass();
	})
...
doTestMergeUniqueH3s( {name:'Add h3 after two',
					   input: [{ h3: '87088619dffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, stationid: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3noChange: 2, h3missing: 1 }],
					   output: [
						   { type: 'day', h3: '87088619bffffff', Count: 2, SumSig: (10>>2)+(10>>2), SumGap: 2, MaxSig: 10},
						   { type: 'day', h3: '87088619dffffff', Count: 1, SumSig: (10>>2), SumGap: 1, MaxSig: 10}]} );

doTestMergeUniqueH3s( {name:'update last h3 after two',
...
doTestMergeUniqueH3s( {name:'update first h3 before two',
...
doTestMergeUniqueH3s( {name:'update middle h3',
...
doTestMergeUniqueH3s( {name:'update all h3s',
					   input: [{ h3: '87088619affffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, stationid: 1 },
					   ...
					   merge: [{ type: 'day', h3source: 3 }, { type: 'day', h3updated: 3, h3noChange:0, h3missing: 0 }],
					   output: [{ type: 'day', h3: '87088619affffff', Count: 3, SumSig: (10>>2)*3, SumGap: 3, MaxSig: 10, MinAlt: 50 },
...
doTestMergeUniqueH3s( {name:'Add h3 in middle',
					   input: [{ h3: '87088619cffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, stationid: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3noChange: 3, h3missing: 1 }],
					   output: [
						   { type: 'day', h3: '87088619cffffff', Count: 1, SumSig: (10>>2), SumGap: 1, MaxSig: 10}]} );

doTestMergeUniqueH3s( {name:'Interleave h3s',
					   input: [{ h3: '87088619a0fffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, stationid: 1 },
					   ...
					   merge: [{ type: 'day', h3source: 5 }, { type: 'day', h3noChange: 4, h3missing: 5 }],
					   output: [
...
doTestMergeUniqueH3s( {name:'Add invalid station (only one in h3)',
					   input: [{ h3: '87088619a1fffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, stationid: 2 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3noChange: 9, h3missing: 1 }],
					   output: [
						   { type: 'day', h3: '87088619a1fffff', Count: 1, SumSig: (10>>2), SumGap: 1, MaxSig: 10}]} );


doTestMergeUniqueH3s( {name:'merge with invalid station in db',
					   input: [{ h3: '87088619a0fffff', altitude: 99, agl: 99, crc: 0, signal: 10, gap: 1, stationid: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3noChange: 8, h3updated: 1, h3stationsRemoved: 1, h3emptied: 1 }],
					   output: [
...
doTestMergeUniqueH3s( {name:'Add invalid station (existing h3)',
					   input: [{ h3: '87088619a0fffff', altitude: 9, agl: 101, crc: 0, signal: 10, gap: 2, stationid: 2 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated:1 }],
					   output: [
						   { type: 'day', h3: '87088619a0fffff', Count: 3, SumSig: (10>>2)*3, SumGap: 4, MaxSig: 10, MinAlt: 9, NumStations: 2}]
/*, finish:true*/} );


doTestMergeUniqueH3s( {name:'merge with invalid station in same h3',
					   input: [{ h3: '87088619a0fffff', altitude: 90, agl: 90, crc: 0, signal: 10, gap: 10, stationid: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated: 1, h3stationsRemoved: 0, h3emptied: 0 }],
					   output: [
						   { type: 'day', h3: '87088619a1fffff', fail: true },
...
// record for id4
doTestMergeUniqueH3s( {name:'invalidate s1',
					   input: [{ h3: '87088619a0fffff', altitude: 91, agl: 91, crc: 0, signal: 10, gap: 10, stationid: 3 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated: 1, h3stationsRemoved: 8, h3emptied: 8 }],
					   output: [
						   { type: 'day', h3: '87088619a1fffff', fail: true },
...
doTestMergeUniqueH3s( {name:'invalidate s3',
					   input: [],
					   merge: [{ type: 'day', h3source: 0 }, { type: 'day', h3updated: 0, h3stationsRemoved: 1, h3emptied: 1 }],
					   output: [
						   { type: 'day', h3: '87088619a1fffff', fail: true },
						   { type: 'day', h3: '87088619b0fffff', fail: true },
						   { type: 'day', h3: '87088619bffffff', fail: true },
...
doTestMergeUniqueH3s( {name:'station:  insert one h3', stationName: '!test',
					   input: [{ h3: '87088619bffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3missing: 1 }],
					   output: [{ type: 'day', h3: '87088619bffffff', Count: 1, SumSig: (10>>2), SumGap: 1, MaxSig: 10}]} );

doTestMergeUniqueH3s( {name:'station: update same h3', stationName: '!test',
					   input: [{ h3: '87088619bffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated: 1 }],
					   output: [{ type: 'day', h3: '87088619bffffff', Count: 2, SumSig: (10>>2)+(10>>2), SumGap: 2, MaxSig: 10}]} );


doTestMergeUniqueH3s( {name:'station:  Add h3 before one', stationName: '!test',
...
doTestMergeUniqueH3s( {name:'station: Add h3 after two', stationName: '!test',
...
doTestMergeUniqueH3s( {name:'station: update last h3 after two', stationName: '!test',
					   input: [{ h3: '87088619dffffff', altitude: 50, agl: 50, crc: 0, signal: 10, gap: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated: 1 }],
					   output: [{ type: 'day', h3: '87088619dffffff', Count: 2, SumSig: (10>>2)+(10>>2), SumGap: 2, MaxSig: 10, MinAlt: 50 }]} );

doTestMergeUniqueH3s( {name:'station: update first h3 before two', stationName: '!test',
					   input: [{ h3: '87088619affffff', altitude: 50, agl: 50, crc: 0, signal: 10, gap: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated: 1 }],
					   output: [{ type: 'day', h3: '87088619affffff', Count: 2, SumSig: (10>>2)+(10>>2), SumGap: 2, MaxSig: 10, MinAlt: 50 }]} );


doTestMergeUniqueH3s( {name:'station: update middle h3', stationName: '!test',
					   input: [{ h3: '87088619bffffff', altitude: 50, agl: 50, crc: 0, signal: 10, gap: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3updated: 1 }],
					   output: [{ type: 'day', h3: '87088619bffffff', Count: 3, SumSig: (10>>2)*3, SumGap: 3, MaxSig: 10, MinAlt: 50 }]} );


doTestMergeUniqueH3s( {name:'station: update all h3s', stationName: '!test',
					   input: [{ h3: '87088619affffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1 },
...
doTestMergeUniqueH3s( {name:'station: Add h3 in middle', stationName: '!test',
					   input: [{ h3: '87088619cffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1 }],
					   merge: [{ type: 'day', h3source: 1 }, { type: 'day', h3noChange: 3, h3missing: 1 }],
					   output: [
						   { type: 'day', h3: '87088619cffffff', Count: 1, SumSig: (10>>2), SumGap: 1, MaxSig: 10}]} );

doTestMergeUniqueH3s( {name:'station: Interleave h3s', stationName: '!test',
					   input: [{ h3: '87088619a0fffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1 },
...
doTestMergeUniqueH3s( {name: 'station: weirding A', stationName: '!test',
					   emptyDb: true,
					   input: [
...
doTestMergeUniqueH3s( {name: 'station: weirding B', stationName: '!test',
					   input: [
						   { h3:'881e019463fffff', altitude: 800, signal: 130, crc: 0, gap: 1 }],
...
doTestMergeUniqueH3s( {name: 'station: weirding C', stationName: '!test',
					   input: [
						   { h3:'881e019463fffff', altitude: 892, signal: 160, crc:0, gap: 1 }],
					   inputAccumulators: { 'current': [ 'current', 1 ], 'day': { bucket: 2 }, 'month': { bucket: 1 } },
					   merge: [
...
doTestStartup( {name: 'A',
				inputAccumulators: { 'current': [ 0, 1 ], 'day': { bucket: 1 }, 'month': { bucket: 1 } },
				currentAccumulators: { 'current': [ 0, 3 ], 'day': { bucket: 2 }, 'month': { bucket: 1 } },
				input: [{ h3: '87088619cffffff', altitude: 100, agl: 100, crc: 0, signal: 10, gap: 1, accumulator: [ 'current', 1] }],
				output: [{ h3: '87088619a0fffff', accumulator: [ 'current', 1 ], fail: true }]});
